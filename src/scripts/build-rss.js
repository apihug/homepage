...
export default async function buildRss() {
  const baseUrl = 'https://apihug.com'
  const blogUrl = `${baseUrl}/blog`

  const feed = new Feed({
    title: 'ApiHug Blog',
    description: 'All the latest ApiHug news, straight from the team.',
    id: blogUrl,
    link: blogUrl,
    language: 'en',
  ...
  posts.forEach(({ slug, module: { meta, default: Content } }) => {
    ...
    const postText = `<p><em>(The post <a href="${blogUrl}/${slug}">${meta.title}</a> appeared first on <a href="${blogUrl}">ApiHug Blog</a>.)</em></p>`
