---
title: Milestone of 2024-05-10
description: With SDK [0.9.9-RELEASE] and plugin  [0.4.0]
---

import { TipBad } from '@/components/Tip'

<TipBad>
  Since `2024-05-10`, this upgrade may introduce code broken!  
</TipBad>
...
1. Format
2. `DEFAULT`: as old framework, convert to `SNAKE` style
3. `CAMEL`: `myVariableName`
4. `SNAKE`: `my_variable_name`
5. Upper
6. `DEFAULT`:as old framework, convert to `UPPER` style
7. `UPPER`: `name` -> `NAME`
8. `LOWER`: `NAME` -> `name`
9. `CAPITALIZE`:  `userName` -> `UserName`
10. Exception: if you set column name manually in the proto already, this will always be the highest priority!
11. `hope.common.persistence.plugin.NameMappingStrategy` client plugin to rename the column name.
