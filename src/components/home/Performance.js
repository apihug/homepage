            ...
            >
              <div className="row-end-1">
                <TabBar
                  side="left"
                  translucent={true}
                  primary={{ name: 'api.proto', saved }}
                  secondary={[
                    { name: 'wire.json' },
                    { name: 'stub.json', open: false, className: 'hidden sm:block' },
                  ]}
                >
                          ...
                          if (token.content === '__CLASSES__') {
                            return (
                              ...
                              >
                                ...
                                {active && (
                                  <Typing
                                    classes="priority: HIGH; //This is very high priority"
                                    rules={rules}
              ...
              <div className="row-span-2 border-t border-slate-500/30 pt-1.5 lg:border-0 lg:pt-0">
                <TabBar
                  side="right"
                  translucent={true}
                  primary={{ name: 'OrderService.java', saved: true }}
                >
                  <svg width="12" height="2" fill="currentColor">
                    <circle cx="1" cy="1" r="1" />
                    <circle cx="6" cy="1" r="1" />
    ...
    >
      ...
      <pre className="flex-auto leading-6 text-sm">
        <code className="block space-y-1 pb-4">
          <div>
            gradlew clean{' '}
            <span className="hidden xl:inline">build</span> -x test{' '}
            <span className="xl:hidden">-w</span> <span className="hidden xl:inline">-x stubTest</span>
          </div>
          {rules.map((_rule, index) => (
            <div key={index} className={index === rules.length - 1 ? 'text-slate-200' : undefined}>
              <span className="code-highlight animate-flash-code-slow">
                BUILD... SUCCESSFUL in {[1025, 886, 1581, 2970, 1664, 2115][index % 6]}ms.
              </span>
            </div>
          ))}
        </code>
